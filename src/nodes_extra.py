...
def planner_node(state: CrewState) -> dict:
    ...
    record = NodeExecutionRecord(node_name="planner", started_at=datetime.now(), model=settings.PLANNER_MODEL)

    req = state["requirements"]
    llm = ChatOpenAI(model=settings.PLANNER_MODEL, temperature=0.2,
                     timeout=settings.REQUEST_TIMEOUT, max_retries=settings.MAX_RETRIES)
    structured = llm.with_structured_output(ResearchPlan)

    msgs = [
        SystemMessage(content=PLANNER_PROMPT.format(topic=req.topic, purpose=req.purpose or "通用调研", audience=req.audience)),
    ...
    try:
        ...
        if req.language == "zh-CN" and plan.questions:
            engs = [q for q in (plan.queries or []) if any(c.isascii() and c.isalpha() for c in q.query)]
            if not engs:
                plan.queries.append(SearchQuery(id=new_id("sq_"), question_id=plan.questions[0].id, query=req.topic, language="en"))

        print(f"  [Planner] {len(plan.questions)}Q / {len(plan.queries or [])}S")
        record.status = "completed"
        record.ended_at = datetime.now()
        return {"research_plan": plan, "current_node": "planner", "node_executions": [record], "updated_at": datetime.now()}

    except Exception as e:
        record.status = "failed"; record.ended_at = datetime.now(); record.error_type = type(e).__name__
        return {"errors": [f"Planner 失败: {e}"], "research_plan": ResearchPlan(thesis=req.topic, questions=[ResearchQuestion(question=req.topic, importance=5)], queries=[SearchQuery(question_id="q_fb", query=req.topic)], completion_criteria=["完成"]), "node_executions": [record]}
...
def outline_builder_node(state: CrewState) -> dict:
    ...
    llm = ChatOpenAI(model=settings.WRITER_MODEL, temperature=0.3,
                     timeout=settings.REQUEST_TIMEOUT).with_structured_output(ReportOutline)

    claims = state.get("claims", [])
    claims_str = "\n".join(f"- [{c.id}] {c.text} (置信度:{c.confidence})" for c in claims[:15])
    msgs = [
        SystemMessage(content=f"你是一名报告结构专家。课题: {state['topic']}\n\n结论:\n{claims_str or '(无)'}\n\n要求:\n1. 覆盖主要维度 2. 每节指定 Claim ID 3. 每节字数 4. 逻辑递进: 背景->现状->分析->结论->局限"),
        HumanMessage(content="生成大纲"),
    ]

    try:
        outline: ReportOutline = llm.invoke(msgs)
        print(f"  [OutlineBuilder] {len(outline.sections)} 节, 约 {outline.estimated_total_words} 字")
        record.status = "completed"; record.ended_at = datetime.now()
