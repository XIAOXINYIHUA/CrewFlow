...
当前版本：`0.3.0` · Python `3.11+` · MIT License
...
- SQLite LangGraph 持久 checkpoint，服务重启后自动恢复 `queued` / `running` 任务
- 运行事件持久化，SSE 支持按 `Last-Event-ID` 或 `after` 游标断点重放
...
- 就绪检查：`GET http://localhost:8000/ready`（验证数据库可用性）
...
API 与 Web 服务共用 PostgreSQL、报告输出卷、来源制品卷，以及保存 LangGraph
SQLite checkpoint 的 `crewflow_checkpoints` 卷。
...
| `CREWFLOW_CHECKPOINT_DB` | `<base>/data/checkpoints/crewflow.sqlite` | LangGraph SQLite checkpoint 文件 |
...
创建任务后，研究图会在后台执行；连接 SSE 端点即可接收已持久化的节点事件。断线后可通过
`Last-Event-ID` 请求头或 `after` 查询参数从指定事件序号继续重放，不会因重连而重新执行研究图。
任务进入 `waiting_human` 后，通过 review 端点提交 `approve`、`revise` 或 `cancel`。

API 进程启动时会扫描 `queued` 和 `running` 任务，并从 SQLite LangGraph checkpoint
自动恢复；尚无 checkpoint 的排队任务会从初始状态启动。

## 开发与验证

```powershell
uv run ruff check src tests migrations main.py web.py
uv run ruff format --check src tests migrations main.py web.py
uv run pytest
uv run mypy src
```
...
- 取消在节点边界生效；已经发出的外部 LLM 请求无法被强制终止，其返回结果会被忽略。
