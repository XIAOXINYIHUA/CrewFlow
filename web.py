...
from src.models import ResearchRequirements, HumanDecision
from src.config import settings
from src.database import init_db
from src.repository import create_run, get_sources, save_human_decision, get_report_versions
...
STAGE_ORDER = [
    "validate_input", "planner", "researcher", "source_processor",
    "claim_builder", "coverage_checker", "outline_builder",
    "analyst", "writer", "citation_checker", "reviewer", "publisher",
]

AUDIENCE_MAP = {"general": "普通读者", "engineer": "工程师", "manager": "管理者", "executive": "高层决策者", "academic": "学术读者"}
...
def _stage_summary(node: str, sources: int = 0, iteration: int = 0) -> str:
    ...
    parts = [f"当前: {label}"]
    if sources: parts.append(f"来源: {sources}")
    if iteration: parts.append(f"第 {iteration} 轮修改")
    return " · ".join(parts)
...
def _fmt_issues(review) -> str:
    ...
    for i in review.issues:
        ...
        if i.paragraph_ref: lines.append(f"  - 位置: {i.paragraph_ref}")
    return "\n".join(lines)
...
def on_cancel(session):
    ...
    return (session, logs, "🛑 任务已取消",
            gr.update(visible=False), gr.update(visible=True),
            gr.update(interactive=True), gr.update(value=1.0))


def on_recover(run_id: str, session: dict):
    """从数据库恢复已运行的任务 (页面刷新后)"""
    if not run_id:
        yield session, gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), "", 0
        return

    from src.repository import get_run
    run = get_run(run_id)
    if not run:
        yield session, gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), "未找到任务", 0
        return

    session.update(run_id=run.id, thread_id=run.thread_id, topic=run.topic,
                   status=run.status, logs="", draft="", source_count=0)

    if run.status == "completed":
        sources_md = _load_sources_md(run.id)
        versions = get_report_versions(run.id)
        report = versions[0].markdown if versions else ""
        yield (session, gr.update(visible=False), gr.update(visible=False), gr.update(visible=True),
               report, sources_md, "✅ 任务已完成", 1.0)
        ...
        rev_md = "### 审查评分\n"
        if review:
            rev_md += (f"- 事实: {review.factuality_score}/100\n- 引用: {review.citation_score}/100\n"
                       f"- 覆盖: {review.coverage_score}/100\n- 结构: {review.structure_score}/100\n"
                       f"- 结论: **{review.verdict}**\n\n### 发现的问题\n{_fmt_issues(review)}")
        yield (session, gr.update(visible=False), gr.update(visible=False), gr.update(visible=False),
               draft, rev_md, "⏸️  等待审批", 0.9)
    else:
        yield session, gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), f"状态: {run.status}", 0
...
def _make_citations_clickable(md: str) -> str:
    ...
    def _replace(m):
        cid = m.group(1)
        return f'<a href="#source-{cid}" style="text-decoration:none;color:#4f46e5;font-weight:500;" title="查看来源 {cid}">[{cid}]</a>'
    return re.sub(r'\[([a-zA-Z0-9_]+)\]', _replace, md)


def _load_sources_md(run_id: str) -> str:
    if not run_id: return "暂无来源数据"
    try:
    ...
    for s in sources[:25]:
        ...
        lines.append(
            f'<div id="source-{s.id}" style="padding:0.5rem;margin:0.3rem 0;border-left:3px solid #e5e7eb;">'
...
def _run_graph(session: dict, resume: dict | None = None):
    ...
    if resume:
        ...
        state = create_initial_state(
            topic=session["topic"], run_id=session["run_id"],
            thread_id=thread_id, requirements=session["requirements"],
    ...
    for event in events:
        for node, upd in event.items():
            ...
            if review:
                logs += f"  📋 {review.verdict} (事实:{review.factuality_score} 引用:{review.citation_score})\n"
            if upd.get("claims"):
                logs += f"  📌 {len(upd['claims'])} 条结论\n"

            yield (session, logs, "", "", gr.update(visible=False),
    ...
    yield (session, logs, "", "", gr.update(visible=False),
           gr.update(visible=False), gr.update(visible=True),
           "✅ 研究完成", gr.update(value=1.0), gr.update(interactive=True))

    # 最终报告
    yield (session, logs, report, sources_md, gr.update(visible=False),
           gr.update(visible=False), gr.update(visible=True),
           "✅ 研究完成", gr.update(value=1.0), gr.update(interactive=True))
...
def on_start(topic, purpose, audience, lang, words, max_src, domains, approval, session):
    """开始研究 (非流式, 准备阶段)"""
    if not topic.strip():
        return (session, "", gr.update(interactive=True), "⚠️ 课题不能为空",
                gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), "", 0)

    lang_code = LANG_MAP.get(lang, "zh-CN")
    preferred = [d.strip() for d in domains.split(",") if d.strip()] if domains else []

    req = ResearchRequirements(topic=topic, purpose=purpose or None,
        audience=audience, language=lang_code, target_words=words,
        max_sources=max_src, preferred_domains=preferred,
        require_human_approval=approval)

    run_id = f"run_{uuid.uuid4().hex[:12]}"
    thread_id = f"thread_{uuid.uuid4().hex[:12]}"

    try: init_db(); create_run(run_id=run_id, thread_id=thread_id, topic=topic, language=lang_code, require_human_approval=approval)
    except Exception as e: print(f"  DB: {e}")

    session.update(run_id=run_id, thread_id=thread_id, topic=topic,
        requirements=req, require_human_approval=approval,
        status="running", logs="", draft="", source_count=0)

    return (session, "", gr.update(interactive=False),
            "🚀 任务已创建，开始研究...",
            gr.update(visible=False), gr.update(visible=True), gr.update(visible=False),
            _stage_summary("planner"), 0.0)
...
def _approve_action(action, feedback, session):
    ...
    if session.get("status") != "waiting_human":
        yield (session, "", gr.update(visible=True), "⚠️ 状态异常，无法审批", "",
               gr.update(visible=True), gr.update(visible=False), "", 0, gr.update(interactive=True))
        return

    if action == "revise" and not feedback.strip():
        yield (session, "", gr.update(visible=True), "⚠️ 退回修改时必须填写修改意见", "",
               gr.update(visible=True), gr.update(visible=False), "", 0, gr.update(interactive=True))
        return

    # 记录
    decision = HumanDecision(action=action, feedback=feedback)
    try: save_human_decision(session["run_id"], decision)
    except: pass
    ...
    if action == "cancel":
        yield (session, "", gr.update(visible=False),
               "🛑 任务已取消", "", gr.update(visible=False), gr.update(visible=True), "", 0, gr.update(interactive=True))
        return
...
def on_export(fmt, report, topic):
    ...
    out = settings.OUTPUT_DIR; out.mkdir(parents=True, exist_ok=True)
    ...
    if "JSON" in fmt:
        ...
        p.write_text(json.dumps({"report": report, "topic": safe, "exported_at": ts}, ensure_ascii=False, indent=2), encoding="utf-8")
        return str(p), f"✅ 已保存: {p.name}"
...
with gr.Blocks(title="CrewFlow", css=CSS, theme=gr.themes.Soft(), fill_height=True) as demo:
    ss = gr.State({"run_id": None, "thread_id": None, "topic": "", "requirements": None,
                   "require_human_approval": True, "status": "idle", "logs": "", "draft": "", "source_count": 0})
    ...
    with gr.Column(visible=True) as input_panel:
        with gr.Tabs() as main_tabs:
            with gr.TabItem("🚀 新研究"):
                with gr.Group():
                    topic_i = gr.Textbox(label="研究课题 *", placeholder="例: 2025年生成式 AI 对软件工程效率的影响", lines=2)
                    with gr.Row():
                        purpose_i = gr.Textbox(label="研究目的", placeholder="内部决策 / 市场分析 / 学术论文", scale=2)
                        audience_i = gr.Dropdown(label="目标读者", choices=[("普通读者","general"),("工程师","engineer"),("管理者","manager"),("高层决策者","executive"),("学术读者","academic")], value="general", scale=1)
                    with gr.Row():
                        lang_i = gr.Dropdown(label="语言", choices=["中文","English","日本語"], value="中文", scale=1)
                        words_i = gr.Slider(label="报告长度", minimum=500, maximum=10000, value=2500, step=500, scale=2)
                    with gr.Accordion("高级设置", open=False):
                        max_src_i = gr.Slider(label="最大来源数", minimum=5, maximum=100, value=30, step=5)
                        approval_i = gr.Checkbox(label="需要人工审批", value=True)
                        domains_i = gr.Textbox(label="首选域名 (逗号分隔)", placeholder="gov.cn, reuters.com, arxiv.org")
    ...
    with gr.Column(elem_classes="container", visible=False) as progress_panel:
        ...
        progress_bar = gr.Progress(value=0, show_label=False)
    ...
    with gr.Column(visible=False) as review_panel:
        ...
        with gr.Group():
            gr.Markdown("### 你的决定")
            feedback_i = gr.Textbox(label="修改意见 (退回时必填)", placeholder="请指明需要补充哪些内容...", lines=2)
            with gr.Row():
                approve_btn = gr.Button("✅ 批准并发布", variant="primary", size="sm", min_width=160)
    ...
    with gr.Column(visible=False) as final_panel:
        ...
        with gr.Row():
            export_fmt = gr.Dropdown(label="格式", choices=["Markdown (.md)", "JSON (.json)"], value="Markdown (.md)", scale=1)
    ...
    click1 = start_btn.click(
        fn=on_start,
        inputs=[topic_i, purpose_i, audience_i, lang_i, words_i, max_src_i, domains_i, approval_i, ss],
        outputs=[ss, start_msg, start_btn, start_msg, input_panel, progress_panel, review_panel, stage_md, progress_bar],
    ).then(
        fn=on_start_stream,
        inputs=[ss],
        outputs=[ss, log_out, draft_md, review_md, review_panel, progress_panel, final_panel, stage_md, progress_bar, start_btn],
    )
    ...
    recover_input.change(
        ...
        outputs=[ss, input_panel, progress_panel, final_panel, final_md, sources_md, stage_md, progress_bar],
    )
    ...
    # 我们需要将所有审批输出合并
    APPROVE_OUTPUTS = [ss, review_status, review_panel, review_panel, final_panel, final_panel,
                       draft_md, review_md, log_out, stage_md, progress_bar, start_btn]
    ...
    approve_btn.click(
        ...
        fn=lambda s: (_load_sources_md(s.get("run_id","")), gr.update(visible=False), gr.update(visible=False), gr.update(visible=True)),
        inputs=[ss],
    ...
    revise_btn.click(
        ...
        inputs=[feedback_i, ss],
        outputs=[ss, review_status, draft_md, review_md, review_panel, progress_panel, final_panel, stage_md, progress_bar, start_btn],
    )
    ...
    cancel_review_btn.click(
        ...
        fn=lambda: (gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), "🛑 任务已取消"),
        outputs=[review_panel, progress_panel, final_panel, final_md],
    )

    # 导出
    export_btn.click(fn=on_export, inputs=[export_fmt, final_md, topic_i], outputs=[export_file, export_msg])
    ...
    def _load_versions(session, report):
        ...
        return gr.update(choices=opts, value=opts[0][0] if opts else ""), gr.update(choices=opts)
